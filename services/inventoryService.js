...
async function getInventoryTableData(userID, query = {}) {
  ...
  const { rows, count } = await InventoryLog.findAndCountAll({
    ...
    limit,
    offset,
    attributes: ['id', 'name', 'brand', 'category', 'type', 'quantity', 'unit', 'packageCount', 'packageUnit', 'quantityPerPackage', 'sellingPrice', 'buyingPrice', 'expiryDate', 'tags', 'voiceResponse', 'createdAt'],
  });
